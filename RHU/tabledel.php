                ...
                <div class="section__content section__content--p30" style="left: -10px" >
                    <div class="container-fluid">
                        ...
                        <div style="padding-left: 830px; position: absolute">
                            <button onclick="deletedata()"><a href=""  class="btn btn-danger">Delete Account</a></button>
                        </div>
                       </div>
...
firebase.auth().onAuthStateChanged(function(user) {
  if (!user) {
    // No user signed in.
     ...
     var tog="foo";
     var tog2="foo1";
     $("#tablebody").append("<tr><td><input type="+cb+" class="+tog+" value="+Id+"></td><td class="+tog2+">"+ Id +"</td><td>"+ Firstname +"</td><td>"+ Lastname +"</td><td>"+ Email +"</td><td>"+ Username +"</td><td>"+ Password +"</td></tr>");
...
function deletedata(){
    ...
    if (confirm("Are you sure you want to delete this?")) {
     ...
     if(checkboxes[i].checked){
        checkedValue = checkboxes[i].value;
        checkedCount++;
        firebase.database().ref("useraccounts").child(checkedValue).remove();
                }
            }
        firebase.database().ref("useraccounts").child(checkedValue).remove().then(() => {
